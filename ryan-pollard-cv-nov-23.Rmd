---
name: Ryan Pollard
position: "Senior Data Scientist and AI Specialist with over eight years experience across different industries."
address: |
         | Manchester, UK
profilepic: rypoll2.png
#www: https://www.upwork.com/freelancers/~01657a3e3e92683896
phone: +44 7500 693 434
email: "rypoll@gmail.com"
github: rypoll
linkedin: rypoll
headcolor: '414141'
date: "`r format(Sys.time(), '%B %Y')`"
output: 
  vitae::awesomecv:
    keep_tex: false
always_allow_html: yes
header-includes:
  \usepackage{float}
  \usepackage{multicol}
  \usepackage{colortbl}
  \usepackage{hhline}

---

```{r setup, include=FALSE}
library(tidyverse)
library(vitae)
library(kableExtra)
library(ggplot2)
library(dplyr)
library(sf)
library(ggrepel)
library(emojifont)
library(fontawesome)
library(huxtable)
library(gridExtra)
knitr::opts_chunk$set(echo = FALSE)
```

# Technical Skills

<!-- \noindent\makebox[\linewidth]{\rule{\textwidth}{0.4pt}} -->

<!-- \par\noindent\nobreak\vspace{-30pt}\rule{\textwidth}{1pt} -->

```{r techtable, results='markup'}
tribble(
   ~`Coding`, ~`Analytics`, ~`Software`,
  "R, Python, SQL, Javascript, HTML, Dart",
  "AI, Machine Learning, Predictive Modeling, NLP",
  "Google Cloud Platform, AWS, Snowflake, Quicksight, Sagemaker, GIT, Flutter, Docker, APIs"
) %>%
  kable(booktabs = T, align = 'c', escape = F, format = "latex") %>%
  kable_styling(full_width = F, font_size = 9,latex_options = "hold_position") %>% 
  row_spec(0, bold = T, color = "#414141") %>% 
  row_spec(1, bold = F, color = "#7f7f7f") %>% 
  column_spec(column = 1:3, width = '4.75cm')
```

[Freelancer profile link: www.upwork.com/freelancers/rypoll](https://www.upwork.com/freelancers/rypoll)

# Current employment

```{r consulting}
tribble(
  ~ role, ~ company, ~ loc, ~ dates, ~ details,
  "Data Scientist", "Freelancer/ Contractor", "Working remotely", "June 2022 - Present", 
    list("Founded own data science agency with a focus on integrating AI to streamline business processes.",
      "Oversaw data migration and integrity for Garner Health's doctor quality analytics using Retool and a bespoke validation system.",
         "Created decision-support dashboards with Snowflake, SQL, and AWS QuickSight for a U.S. healthcare provider quality reporting service."

         #,
         #"Hosting workshops improving data literacy, development, analysis and modelling capabilities."
    )
  #,
  # "Research assistant", "Monash University", "Clayton, Australia", "Oct. 2015 - Present", 
  #   list("Involved in many internal and external data projects of varying size and complexity.", 
  #        "Projects typically feature large scale time series modelling or analysis, cross-sectional modelling and creating tools for improving workflows.",
  #        "Consulting project clients include: Huawei, Monash University, R Consortium, NSW Chief Scientist office, DiabetesLab and Tennis Australia."),
  # "Teaching Associate", "Monash University", "Clayton, Australia", "Mar. 2016 - Present", "Sessional teaching of university students undertaking Bachelor's and Master's degrees. Specific classes are listed in 'Teaching experience'."
  # "Data mining intern", "iSelect", "Cheltenham, Australia", "Feb. 2015 - Mar. 2015", "Improved business reporting with interactive visualisations, and model-based anomaly detection.",
) %>% 
  detailed_entries(role, dates, company, loc, details)
```



# Work Experience

```{r workxp}
tribble(
  ~ position, ~ workplace, ~ loc, ~ dates, ~ details,
  
    "Senior Data Scientist", "Tinka (Fintech)", "Remote", "Feb 2022 - May 2023", 
    list("Tinka in a deferred payments company applying state of the art machine learning techniques on large datasets.",
         "Built from start to end a Fraud model leading to a decrease in Fraud cases and increase in profitability. Built and deployed in AWS.",
         "Built customer risk model",
         "Simplified and standardised the model build process",
         "Built dashboards using DataBricks for various A/B Tests on goingin the business",
         "Technical skills used: Python, SQL, Spark, AWS, Databricks, Sagemaker."),
         #,
         
  
  "Data Scientist", "Freelancer w/ various clients", "Remote", "Jun. 2021 - Feb. 2022", 
    list("Deployed predictive models and data science applications with a 100pc success rate; specialized in NLP and machine learning.",
  "Engineered a neural network for gene prediction and interactive R Shiny dashboards for data analysis.",
  "Developed tools for medical literature analysis and predictive models for seller success on an auction platform."

),
  
  
  "Mathematics Teacher | Traveling", "Various schools", "Colombia | China", "Jan. 2018 - July 2020", 
    list("Teaching Mathematics in Colombia and China.",
         "Travelling through Central/South America and Asia."

),

  "Senior Risk Modeler", "The Co-Operative Bank", "Manchester, United Kingdom", "Feb. 2016 - July 2017", 
    list("Leading projects, communicating progress and new ideas with stakeholders, planning and executing the data-lead projects.",
"Developed and maintained credit risk models, enhanced model understanding and performance within portfolios, and managed dashboard creation and analytics."
),

  "Preditctive Modeler", "British Gas", "Manchester, United Kingdom", "Mar. 2014 - Feb. 2016", 
    list( "Constructed scorecards, optimized communication strategies for debt recovery, developed management information systems, and improved business processes through model enhancements."
      
    ),

  "Statistician", "Adelphi Real World", "Manchester, United Kingdom", "June 2011 - October 2013", 
    list( "Conducted statistical analyses on medical data for publication, ensuring accuracy and adherence to model assumptions, and prepared data for testing."
          
      
    )

) %>% 
  detailed_entries(position, dates, workplace, loc, details)
```

# Education

```{r education}
tribble(
  ~ degree, ~ uni, ~ loc, ~ dates,
  "MSc Statistics", "University of Sheffield", "Sheffield, United Kingdom", "Sept. 2020 - Sept. 2021",
  "BSc Mathematics", "University of Manchester", "Manchester, United Kingdom", "Sept. 2007 - Sept. 2010"

) %>% 
  detailed_entries(degree, dates, uni, loc)
```



# Interests

- Muay Thai
- Bass Guitar
- Reading




# Languages

-   Spanish: B2 (Common European Framework of Reference for Languages: A1/A2: Basic User. B1/B2: Independent User. C1/C2: Proficient User)

```{r langforeign, results='markup'}
# data.frame(
#   Skill = c("Reading", "Writing", "Listening","Speaking"),
#   English = cell_spec(c(rep("Native", 4)), 'latex', color = '#414141'), 
#   Spanish = cell_spec(c("C1","B2","B2","B2"), 'latex', color = '#414141')
# ) %>%
#   kable(booktabs = T, align = 'c', escape = F, format = "latex") %>%
#   kable_styling(full_width = F, font_size = 9) %>% 
#   footnote(
#     footnote_as_chunk = T,
#     general_title = " ",
#     general = "\\\\tiny Common European Framework of Reference for Languages: A1/A2: Basic User. B1/B2: Independent User. C1/C2: Proficient User",
#     escape = F
#   ) %>%
#   column_spec(column = 1:2, width = '2.4cm') #%>% 
#   # add_header_above(' ', line = F) %>%
#   # row_spec(0:4, extra_latex_after = "\\arrayrulecolor{white}") 
```

<!-- \smallskip -->
